...
def main():
    ...
    with tf.name_scope('final_training_ops'):
        weights1 = tf.Variable(tf.truncated_normal([BOTTLENECK_TENSOR_SIZE, 512], stddev=0.001))
        biases1 = tf.Variable(tf.zeros(512))
        y1 = tf.matmul(bottleneck_input, weights1) + biases1
        logits1 = tf.nn.sigmoid(y1)

        weights2 = tf.Variable(tf.truncated_normal([512, n_classes], stddev=0.001))
    ...
    with tf.Session() as sess:
        train_writer = tf.summary.FileWriter("logs/sigmoid_qyxx3/train", sess.graph)
        vali_writer = tf.summary.FileWriter("logs/sigmoid_qyxx3/vali", sess.graph)
